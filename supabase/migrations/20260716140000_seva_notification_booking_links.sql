/* Seva — Step 4 follow-up: every notification deep-links to its own booking.
   Run AFTER 20260716130000. */

-- /bookings can't target one booking, so a status notification could only ever land you on a
-- list and leave you to find the row. Booking actions now live on /bookings/[id] alongside
-- the chat, so every notification points at the specific booking it's about. This supersedes
-- the ?role= querystring from 20260716130000 — role is derived from the booking itself now.
...
-- 3) New message → notify the recipient. ?tab=chat scrolls the detail page to the thread;
--    status notifications omit it and open at the top, where the action buttons are.
...
BEGIN
  ...
  IF v_recipient IS NOT NULL AND v_recipient <> NEW.sender_id THEN
    ...
    VALUES (v_recipient, 'New message', 'You have a new message about a booking.', 'info',
            '/bookings/' || NEW.booking_id || '?tab=chat');
  END IF;
  RETURN NEW;
END; $$;
