...
- **`/services` hardcodes its own category chips.** The page carries a 14-entry list with its own labels while `service_categories` holds 25 rows, so some categories cannot be filtered from that page at all, and the labels drift from the DB names ("Farm Fresh" vs "Farm Fresh Delivery"). Pre-dates Step 11; surfaced when a test clicked a DB category name and found no such button. The chips should be driven from `service_categories`.
- **Text search on `/providers` filters the returned page, not the query.** The same shape as the category bug above, and not yet fixed: typing "electrician" filters the 30 rows the RPC returned rather than searching all in-range providers. Lower impact (it is a free-text convenience, not a primary filter) but it will mislead the same way at density.
