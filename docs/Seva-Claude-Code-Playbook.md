...
**✅ SHIPPED** (migrations `20260818120000` + `20260818130000`): `search_providers` RPC, both listing pages ranking through it, provider service base at onboarding. The stored `geo` point is **grid-snapped to ~250 m** — returning distance from an attacker-chosen origin otherwise trilaterates a provider's home, so "distance only" was not by itself the safety property the spec assumed. Details and the `anon` definer-grant finding are in the Decisions Log. `exploration_bonus` is NOT built — it is still open here.
